...
Al llarg del document estudiarem i representarem les distàncies de les representacions en grafs dels diferents metabolismes que presenten els animals observats. En particular, el que farem serà estudiar les distàncies obtingudes de les similituds entre VH, SP, PM, i les representarem a partir de dues coordenades principals.Per finalitzar, compararem l'agrapació obtinguda i l'agrupació que s¡obté amb el phylum, coloretjant els puns en aquesta darrera agrupació.

Començam llegint el fitchet adjuntat a aula digital i definim la variable phylum.

```{r}
phylum <- read.csv("Datos para el ejercicio 2 de la práctica 6 (ACP y MDS)-20241213/fileListANIMAL_phylum.txt")
```

En primer lloc, començarem amb la matriu de distàncies obtinguda a partir del métode VH, i mostrarem la matriu de partida, la qual és una ,atriu de similituds entre els primers 15 grafs. 

```{r}
mat_vh = read.table("Datos para el ejercicio 2 de la práctica 6 (ACP y MDS)-20241213/ANIMALS-matrixVH.txt")
