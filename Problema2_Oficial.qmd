...
Matriu de similituds obtinguda amb el mètode SP: vam mostrar les relacions entre els 15 primers grafs:
```{r, include=FALSE}
...
A diferència de la matriu de similituds VH, sembla que aquí hi ha una millor distinció, vegem si és cert, calculem la matriu de distàncies:
...
Tornem a aplicar el mètode de coordenades principals amb la funció de R `cmdscale`, ara amb la matriu de distància anterior i especificant $k = 2$ per a les coordenades principals. Les primeres 15 observacions són:
...
I finalment vam mostrar el gràfic de la matriu de similitud generada pel mètode SP diferenciant una altra vegada pel grup d'animals que pertanyen segons el seu phylum amb el mateix mètode anterior.
...
```{r}
ggplot(sp_mds, aes(x = Dim.1, y = Dim.2, color = phylum1)) + geom_point(size = 2) +  
  labs(title = "Representació del MDS", x = "Dimensió 1", y = "Dimensió 2", color = "Phylum") +
  theme_minimal()
```
